...
def run_ueba_models():
    ...
    anomalies = []
    query = "WITH daily AS (SELECT hostname, date_trunc('day', CAST(timestamp AS TIMESTAMP)) as day, count(*) as c FROM siem.events WHERE CAST(timestamp AS TIMESTAMP) >= CURRENT_DATE - INTERVAL 30 DAY GROUP BY 1, 2), stats AS (SELECT hostname, avg(c) as avg_c, stddev_pop(c) as sd_c FROM daily WHERE day < CURRENT_DATE GROUP BY 1), today AS (SELECT hostname, count(*) as cur_c FROM siem.events WHERE CAST(timestamp AS TIMESTAMP) >= CURRENT_DATE GROUP BY 1) SELECT t.hostname, t.cur_c, s.avg_c, s.sd_c FROM today t JOIN stats s ON t.hostname = s.hostname WHERE t.cur_c > (s.avg_c + (3 * COALESCE(s.sd_c, 0))) AND s.avg_c > 50"
