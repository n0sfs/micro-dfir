...
CREATE TABLE IF NOT EXISTS alerts (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, rule_id INTEGER, event_id INTEGER, rule_name TEXT, host TEXT, message TEXT, severity TEXT, acknowledged BOOLEAN DEFAULT 0, username TEXT, source_ip TEXT, destination_ip TEXT, log_event_id TEXT, log_app TEXT, occurrence_count INTEGER DEFAULT 1, last_seen DATETIME, FOREIGN KEY(rule_id) REFERENCES sigma_rules(id), FOREIGN KEY(event_id) REFERENCES live_logs(id));
